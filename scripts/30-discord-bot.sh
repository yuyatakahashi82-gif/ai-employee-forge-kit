...
[ -z "$UV" ] && { log "FAIL: uv が見つからない"; exit 1; }
if ! "$VENV_PY" -c "import discord" >/dev/null 2>&1; then
  log "discord.py 導入中..."
  "$UV" pip install --python "$VENV_PY" -q "discord.py[voice]==2.7.1" "aiohttp==3.13.4" "brotlicffi==1.2.0.1" || { log "FAIL: discord.py 導入"; exit 1; }
fi
"$VENV_PY" -c "import discord" >/dev/null 2>&1 && log "discord.py OK" || { log "FAIL: discord import"; exit 1; }
